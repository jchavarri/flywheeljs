...
if (!process.argv.slice(2).length) {
  ...
  const tip = program.tip || 500;
  const notes = program.notes || '';
  var authToken = undefined;
  var passenger = undefined;

  if (program.email && program.password && program.address) {
    var addressData = {}; // Will fill this object with google geocoding results
    const googleUrl = 'http://maps.googleapis.com/maps/api/geocode/json?address=' + encodeURIComponent(program.address);
    axios.get(googleUrl)
    .then(function (response) {
      const results = response.data.results;
      if (!results || results.length === 0) {
        return Promise.reject('We couldn\'t find any addresses corresponding to \'' + program.address + '\'. Use a comma separated format including the state, for example: --address \'22 Estate Ct, South San Francisco, CA\'');
      }
